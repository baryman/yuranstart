...
if (class_exists('statusOrder')) {
  ...
  while ($dbRes = DB::fetchArray($dbQuery)) {
    ...
    $listStatusTemp[$dbRes['id_status']] = $dbRes['status'];
  }
  $listStatus = $statusOrder;
} else {
...
if (strtotime($minDate) >  strtotime($maxDate)) {
  $maxDate = $minDate;
}
...
$this->assocStatus = !empty($statusOrder) ? $statusOrder : $ls;
$this->assocStatusClass = array('dont-confirmed', 'get-paid', 'paid', 'in-delivery', 'dont-paid', 'performed', 'processed'); // цветная подсветка статусов
$model = new Models_Order();
