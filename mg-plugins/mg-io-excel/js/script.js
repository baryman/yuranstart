  ...
  return { 
    ...
    init: function() {      
      ...
      // Клик по кнопкам таб панели
			$('.admin-center').on('click', '.section-'+mgIOExcel.pluginName+' .tabs-list li', function() {			
        $('.tabs-list li').removeClass('is-active');
        $(this).addClass('is-active');
        $('.widget-table-action .setting-block').hide();
        $('.section-'+mgIOExcel.pluginName+' .'+$(this).find('a').attr('id').replace('tab', 'block')).show();
			});
      ...
      $('.admin-center').on('change', 'select[name=importType]', function(){
        if($(this).val() != 0){
          ...
          $('label[for=ExcelChoser]').removeAttr('disabled');
          ...
          $('label[for=ExcelChoser]').attr('disabled', 'disabled');
        }
      });
    ...
    showModal: function(type){
      ...
      var obj = '{';
      $('.setting-import-block input, .setting-import-block select').each(function(){     
    ...
    fillField: function(){
      return function(response){
        ...
        response.data.maskArray.forEach(function(item, i, arr){
          ...
          $('.b-modal tbody').append(compList);
          $('.b-modal select[name=colIndex'+i+'] option[value='+response.data.compliance[i]+']').attr('selected', 'selected');
        });
...
//          admin.show(mgIOExcel.pluginName, "plugin");
    ...
    importCatalog: function(nextRow, step){
      var obj = '{';
      $('.base-settings input, .base-settings select').each(function(){     
        obj += '"' + $(this).attr('name') + '":"' + $(this).val() + '",';
      });
      obj += '}';    
