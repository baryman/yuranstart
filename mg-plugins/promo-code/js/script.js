...
var promoCode = (function() {
  ...
  return { 
    ...
    init: function() {      
      ...
      $('.admin-center').on('click', '.section-promo-code #promo-modal .save-button', function() { 
        var id = $(this).data('id');    
    ...
    */         
    getFile: function(file) {      
      $('.section-promo-code #promo-modal  input[name="src"]').val(file.url);
    },      
    ...
    */         
    clearField: function() {        
      $('.section-promo-code #promo-modal .fields-calback input[name="code"]').val('');  
      $('.section-promo-code #promo-modal .fields-calback input[name="percent"]').val('');
      $('.section-promo-code #promo-modal .fields-calback input[name="from_datetime"]').val('');  
      $('.section-promo-code #promo-modal .fields-calback input[name="to_datetime"]').val('');
      $('.section-promo-code #promo-modal .fields-calback textarea[name=desc]').val('');
      $('.section-promo-code #promo-modal .save-button').data('id','');        
    },
    ...
    fillField: function(id) {
      ...
      function(response) {         
        $('.section-promo-code #promo-modal .fields-calback input[name="code"]').val(response.data.code);  
        $('.section-promo-code #promo-modal .fields-calback input[name="percent"]').val(response.data.percent);
        $('.section-promo-code #promo-modal .fields-calback input[name="from_datetime"]').val(response.data.from_datetime);  
        $('.section-promo-code #promo-modal .fields-calback input[name="to_datetime"]').val(response.data.to_datetime);
        $('.section-promo-code #promo-modal .fields-calback textarea[name=desc]').val(response.data.desc);     
        $('.section-promo-code #promo-modal .save-button').data('id',response.data.id);     
      },
              
      $('#promo-modal .widget-table-body') // вывод лоадера в контейнер окна, пока идет загрузка данных
    ...
    saveField: function(id) {
      ...
      if($('.section-promo-code #promo-modal .fields-calback input[name="percent"]').val() <0 ||
         $('.section-promo-code #promo-modal .fields-calback input[name="percent"]').val() >100 ||
         isNaN(parseFloat($('.section-promo-code #promo-modal .fields-calback input[name="percent"]').val()))){
      ...
      admin.ajaxRequest({
        ...
        pluginHandler: 'promo-code', // плагин для обработки запроса
        id: id,
        code: $('.section-promo-code #promo-modal .fields-calback input[name="code"]').val(),
        percent: $('.section-promo-code #promo-modal .fields-calback input[name="percent"]').val(),
		    from_datetime: $('.section-promo-code #promo-modal .fields-calback input[name="from_datetime"]').val(),   
        to_datetime: $('.section-promo-code #promo-modal .fields-calback input[name="to_datetime"]').val(),
        desc:   $('.section-promo-code #promo-modal .fields-calback textarea[name=desc]').val(), 
        invisible: invisible,  
      },
      ...
      $('#promo-modal .widget-table-body') // на месте кнопки
       ...
       <tr data-id="'+data.id+'">\
        ...
        <td class="actions">\
           <ul class="action-list">\
             <li class="edit-row" data-id="'+data.id+'" data-type="'+data.type+'"><a class="tool-tip-bottom  fa fa-edit" href="javascript:void(0);" title="'+lang.EDIT+'"></a></li>\
             <li class="visible tool-tip-bottom '+invisible+'" data-id="'+data.id+'" title="'+titleInvisible+'"><a class="visible fa fa-lightbulb-o" href="javascript:void(0);"></a></li>\
