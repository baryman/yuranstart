  ...
  function __construct() {
    ...
    if (!empty($_REQUEST['type'])) {
      ...
      if ($_REQUEST['type'] == 'recommend') {
        ...
        $items = $model->getListByUserFilter(MG::getSetting('countСatalogProduct'), ' p.recommend = 1 and p.activity=1 '.$onlyInCount.' ORDER BY sort ASC');
        ...
        $items = $model->getListByUserFilter(MG::getSetting('countСatalogProduct'), ' p.new = 1 and p.activity=1 '.$onlyInCount.' ORDER BY sort ASC');
        ...
        $items = $model->getListByUserFilter(MG::getSetting('countСatalogProduct'), ' (p.old_price>0 || pv.old_price>0) and p.activity=1 '.$onlyInCount.' ORDER BY sort ASC');      
      }
