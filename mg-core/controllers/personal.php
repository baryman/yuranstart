  ...
  function __construct() {
    ...
    if (User::isAuth()) {
      ...
      if (URL::getQueryParametr('delOK')) {
        ...
        if (!$status) {
          $status = $lang[$order->getOrderStatus(array('status_id' => 4))];
        }
        $result = array(
          'status' => $status,
          'comment' => $comm,
      ...
      if (is_array($orderArray)) {
        foreach ($orderArray as $orderId => $orderItems) {
          $orderArray[$orderId]['string_status_id'] = $statusOrder[$orderItems['status_id']] ? $statusOrder[$orderItems['status_id']] : $lang[$order->getOrderStatus($orderItems)];
