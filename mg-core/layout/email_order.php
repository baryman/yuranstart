...
<?php // echo print_r($data,1); ?>
     ...
     <?php /* if(!USER::getUserInfoByEmail($data['email'])->activity) { ?>
      ...
      <p class="confirm-info" style="font-size:12px;margin:0 0 10px 0; text-align: center;">
        <a style="text-transform: uppercase; height: 48px; padding: 0 35px; font-size: 18px; line-height: 46px;border: 1px solid #44B926;color: #fff;background: #44B926;display: inline-block;transition: all 0.2s ease-in-out;   text-decoration: none;  outline: none; box-sizing: border-box;" href="<?php echo SITE.'/order?orderID='.str_replace('M-010','',$data['orderNumber']).'&orderSumm='.$data['total'].'&paymentId='.$data['paymentId'].'&pay=go'; ?>" >Оплатить заказ сейчас</a>
      </p>
