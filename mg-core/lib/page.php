...
class Page {
   ...
   *
   * @param int $pageUrl заданная страница
   * @return array
   */
  ...
  /**
   *  Упорядочивает страниц по сортировке.
   */
  ...
  public function getPages($pagesArray, $parentLevel, $parent) {
    ...
    foreach($pagesArray as $page) { 
      $pageCount = MG::get('pageCountToAdmin');
          if(($page['parent'] == 0) && ($parentLevel == 0)) {
            MG::set('pageCountToAdmin',++$pageCount);
            $pages .= self::getHtmlPageRow($pagesArray, $page['id'], $parentLevel);
            $pages .= self::getPages($pagesArray, $parentLevel+1, $page['id']);
            
          }
          if (($page['parent'] != 0) && ($page['parent'] == $parent)) {
            MG::set('pageCountToAdmin',++$pageCount);
            $pages .= self::getHtmlPageRow($pagesArray, $page['id'], $parentLevel);
            $pages .= self::getPages($pagesArray, $parentLevel+1, $page['id']);
          }
  ...
  public function getHtmlPageRow($pages, $id, $level) {
    ...
    foreach($pages as $page) { 
      if($page['id'] == $id) {
        ...
        // если вложеные страницы, то изначально не показываем
        if($level > 0) {
        ...
        return '
              <tr class="level-'.($level+1).' '.$group.'" '.$hide.' data-id="'.$page['id'].'" data-group="'.$group.'" data-level="'.($level+1).'" data-sort="'.$pageCount.'">
                <td class="checkbox">
                  <div class="checkbox">
                ...
                <td class="text-right actions">
                  <ul class="action-list">
                    ...
                    <li><a class="fa fa-plus-circle tip add-sub-cat" href="javascript:void(0);" aria-hidden="true" title="Добавить вложенную страницу"></a></li>
