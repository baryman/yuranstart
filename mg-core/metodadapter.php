 ...
 * @param  $userFunction пользовательская функции, которая сработает при объявлении хука.
 ...
 * @param $userFunction пользовательская функции, которая сработает при встрече [названия шорткода].
 */
 ...
 * @param  $userFunction пользовательская функция, которая сработает при объявлении хука.
 */
...
function mgImageProduct($data){
  ...
  return '<img class="mg-product-image" data-transfer="true" data-product-id="'.$data["id"].'" src="'.$src.'" alt="'.$imagesData["image_alt"].'" title="'.$imagesData["image_title"].'" data-magnify-src="'.$srcLarge.'">';
}
