...
var category = (function() {
  return {
    ...
    init: function() {
      ...
      $('body').on('click', '.section-category #add-category-modal .html-content-edit', function() {
        var link = $(this);
      ...
      $('body').on('click', '#add-category-modal .add-image-to-category', function() {
        admin.openUploader('category.getFile');

      });  
      
       // Удаление картинки категории
      $('body').on('click', '#add-category-modal .del-image-to-category', function() {         
     ...
     // Выделить все категории.
            ...
            <tr id="loader-'+id+'">\
              ...
              <td class="text-right actions">\
                <ul class="action-list">\
                  ...
                  <li><a class="fa fa-plus-circle tip add-sub-cat" href="javascript:void(0);" aria-hidden="true" title="Добавить вложенную категорию"></a></li>\
                  <li><a class="fa fa-lightbulb-o tip activity" href="javascript:void(0);" aria-hidden="true" title="Отображение"></a></li>\
                  <li><a class="fa fa-list tip visible" href="javascript:void(0);" aria-hidden="true" title="Товары в категории"></a></li>\
                  <li><a class="fa fa-shopping-cart tip export" href="javascript:void(0);" aria-hidden="true" title="Включить в выгрузку на Яндекс.Маркет"></a></li>\
      ...
      $('body').on('click', '#add-category-modal .discount-setup-rate', function() {
        $(this).hide();
      ...
      $('body').on('click', '#add-category-modal .cancel-rate', function() {
      ...
      $('body').on('click', '#add-category-modal .change-rate', function() {
        $('.select-rate-block').show();
        $('.discount_apply_follow').show();
      }); 
      // Клик по кнопке для  отмены модалки смены скидки/наценки
      $('body').on('click', '#add-category-modal .cancel-rate-dir', function() {
      ...
      $('body').on('click', '#add-category-modal .apply-rate-dir', function() {
        $('.select-rate-block').hide();      
      ...
      CKEDITOR.on('instanceCreated', function(e) {        
        if (e.editor.name === 'html_content') {
          ...
          if (!$.trim(description)) {            
            description = $('#add-category-modal textarea[name=html_content]').val();
    ...
    */
    generateSeoFromTmpl: function() {
      ...
      if (!$('.seo-wrapper input[name=meta_title]').val()) {
        $('.seo-wrapper input[name=meta_title]').val($('#add-category-modal input[name=title]').val());
      }
      ...
      if (!$('.seo-wrapper textarea[name=meta_desc]').val()) {
        ...
        $('#add-category-modal .seo-wrapper textarea[name=meta_desc]').val($.trim(short_desc));
      }
      ...
      admin.ajaxRequest({
        ...
        $('#add-category-modal .seo-wrapper textarea[name=meta_desc]').trigger('blur');
        admin.indication(response.status, response.msg);
      });
    },
    ...
    startImport:function(rowId, percent) {
      ...
      if(!catalog.STOP_IMPORT){
        $('.message-importing').html('Идет процесс импорта категорий. Загружено:'+percent+'%<div class="progress-bar"><div class="progress-bar-inner" style="width:'+percent+'%;"></div></div>');
      }else{
        $('.loading-line').remove();
      }
      ...
      function(response){
        ...
        if(response.data.percent<100){
          if(response.data.status=='canseled'){
           ...
           $('.message-importing').html('Импорт категорий успешно завершен! \
    ...
    addCategoryToOpenArr: function(id, level = 0) {
      if(category.openedCategoryAdmin[level] == undefined) {
        category.openedCategoryAdmin[level] = [];
      }

      var addId = true;
      category.openedCategoryAdmin[level].forEach(function(item) {
        if (item == id) {
          addId = false;
        }
      });
      ...
      if (addId) {
        category.openedCategoryAdmin[level].push(id);
      }
    ...
    delCategoryToOpenArr: function(id, level = 0) {
      if(category.openedCategoryAdmin[level] == undefined) {
        category.openedCategoryAdmin[level] = [];
      }
      ...
      if (dell) {
        category.openedCategoryAdmin[level].splice(spliceIndex, 1);
      }
    ...
    saveCategory: function(id) {
      ...
      var packedProperty = {
        ...
        image_url: $('#add-category-modal input[name="image_url"]').val(),
        invisible: $('#add-category-modal input[name=invisible]').val() == 'true' ? 1 : 0,
        rate: rate,
    ...
    fillFields: function() {
      return (function(response) {
        ...
        if(response.data.image_url){
          $('#add-category-modal .category-image').attr('src', admin.SITE+response.data.image_url);
          $('#add-category-modal .category-image').show();
          $('#add-category-modal .category-img-block').show();          
          $('#add-category-modal .del-image-to-category').show();  
          $('#add-category-modal .add-image-to-category').hide();
          $('#add-category-modal  input[name="image_url"]').val(admin.SITE+response.data.image_url);
        } else{
           $('#add-category-modal  input[name="image_url"]').val('');
    ...
    */         
    getFile: function(file) {      
      $('#add-category-modal  input[name="image_url"]').val(file.url);
      $('#add-category-modal .category-image').attr('src',file.url);
      $('#add-category-modal .category-img-block').show();
      $('#add-category-modal .category-image').show();
      $('#add-category-modal .add-image-to-category').hide();
      $('#add-category-modal .del-image-to-category').show();  
    ...
    hidePageRows: function(level = 2) {
      ...
      for(var i = 0; i < category.openedCategoryAdmin[level].length; i++) {
        if(category.openedCategoryAdmin[level][i] != undefined) {
          $('#toHide-'+category.openedCategoryAdmin[level][i]).click();
        }
      }
    },
