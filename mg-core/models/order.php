...
class Models_Order {
  ...
  public function sendMailOfPayed($orderNamber, $paySumm, $pamentId) {
    ...
    $res = DB::query('SELECT cost FROM `'.PREFIX.'delivery` AS t1
                        LEFT JOIN '.PREFIX.'order AS t2 ON t2.delivery_id = t1.id
                        WHERE t2.id = '.DB::quote($orderNamber));
  ...
  public function getExportCSV($orderId) {
    ...
    foreach ($order_content as $item) {
      $csvText .= "\"".$item["id"]."\";\"".$order["add_date"]."\";\"".$order["name_buyer"]."\";\"".$order["user_email"]."\";\"".$order["phone"]."\";\"".$order["address"]."\";\"".$order["comment"]."\";\"".$item["name"]."\";\"".$item["code"]."\";\"".number_format($item["price"]*$item["count"], 2, ',', '')."\";\"".$item["count"]."\";\"".$item["coupon"]."\"\n";
    }
    ...
    if ($params['promocode']) {
      ...
      if (DB::numRows($result)) {
        ...
        if ($code = DB::fetchAssoc($res)) {
          ...
          $disountProduct = 'SELECT GROUP_CONCAT(id) AS id FROM `'.PREFIX.'product` WHERE `cat_id` in ('.DB::quote($code['category'],true).') AND id IN ('.DB::quote($productId,true).')';
