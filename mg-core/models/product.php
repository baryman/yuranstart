  ...
  public function addProduct($array, $clone = false) {
    ...
    if (DB::buildQuery('INSERT INTO `'.PREFIX.'product` SET ', $array)) {
      ...
      if ($dublicatUrl) {
        ...
        $updateArray = array('id' => $id, 'url' => $array['url'].'_'.$id, 'sort' => $id);
        if ($clone) {
          $updateArray['code'] = PREFIX_CODE.$id;          
          $array['code'] = PREFIX_CODE.$id;
        }
        $this->updateProduct($updateArray);
      } else {
        $updateArray = array('id' => $id, 'url' => $array['url'], 'sort' => $id);
        if ($clone) {
          $updateArray['code'] = PREFIX_CODE.$id;
          $array['code'] = PREFIX_CODE.$id;
        }
   ...
   // if (count($variants) > 1) {
      ...
      foreach ($variants as $variant) {
        ...
        if (empty($variant['code'])) {
          $variant['code'] = PREFIX_CODE.$id.'_'.$i;
        }
        $variant['sort'] = $i++;
