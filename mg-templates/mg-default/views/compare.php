...
<div class="mg-compare-products">
    ...
    <div class="mg-compare-left-side">
        <?php if (!empty($_SESSION['compareList'])) { ?>
            <?php if (MG::getSetting('compareCategory') != 'true') { ?><h2>Показать сравнение из
                категории:</h2><?php } ?>
            <div class="mg-category-list-compare">
                ...
                <?php } ?>
                <a href="<?php echo SITE ?>/compare?delCompare=1" class="mg-clear-compared-products">Очистить весь
                    список сравнений</a>
                <a href="<?php echo SITE ?>/catalog" class="more-models-btn">Добавить другие модели</a>
            </div>
        <?php } ?>
                ...
                if (!empty($data['catalogItems'])) {
                    foreach ($data['catalogItems'] as $item) {
                        ...
                        <div class="mg-compare-product">
                            <a class="mp-remove-compared-product"
                               href="<?php echo SITE ?>/compare?delCompareProductId=<?php echo $item['id'] ?>"></a>
                            <div class="mg-compare-product-inner">
                                <div class="title"><a
                                        href="<?php echo $item['link'] ?>"><?php echo $item['title'] ?></a></div>
                                ...
                                <ul class="mg-compare-product-list product-status-list">
                                    <li><span
                                            class="price"><?php echo $item['price'] ?><?php echo $item['currency']; ?></span>
                                    </li>
                                    <li <?php echo (!$item['old_price']) ? 'style="display:none"' : 'style="display:block"' ?>>
                                        <span
                                            class="old-price"><?php echo $item['old_price'] . " " . $item['currency']; ?></span>
                                    </li>
                                </ul>
    ...
    <div class="mg-compare-fake-table">
        <div class="mg-compare-fake-table-left <?php echo $data['moreThanThree'] ?>">
            <?php foreach ($propTable as $key => $prop) { ?>
                <div
                    class="mg-compare-fake-table-cell <?php if (trim($data['property'][$key]) !== '') : ?>with-tooltip<?php endif; ?>">
