  ...
  switch ($data['step']) {
                  ...
                  <?php foreach ($data['delivery'] as $delivery): ?>
                    <li <?php echo ($delivery['checked']) ? 'class = "active"' : 'class = "noneactive"' ?>>
                      <label data-delivery-date="<?php echo $delivery['date']; ?>">
                        <input type="radio" name="delivery" <?php if ($delivery['checked']) echo 'checked' ?>
    ...
    <?php else: ?>
      <div class="payment-form-block">
        ...
        <hr>[sms]Поступил заказ #<?php echo $data['orderNumber']; ?> Сумма <?php echo $data['summ']; ?> <?php echo $data['currency']; ?> Способ доставки: <?php echo $data['delivery'][$_POST['delivery']]['description']; ?>[/sms]
		...
		<?php if($data['pay'] ==13):?>
			<p>Для того, что, бы мы отправили Вам заказ, Вы должны осущиствить полную оплату заказа (<b>100% предоплата</b>). Осуществите перевод на карту Сбербанка. Номер карты Сбербанка - <b>5469 4000 1393 7219</b></p>
		<?php endif;?>
      <?php
    endif;
    ...
    case 3:
      ...
      if ($data['active']):
        ?>
        <div class="text-success">Вы успешно зарегистрировались на сайте <strong><?php echo SITE ?></strong> и можете
    ...
    case 4:
      ...
      <h1 class="new-products-title">Оплатите заказ № <?php echo $data['orderNumber'] ?> на
      ...
      if ($data['payMentView']) {
		...
		<p>Если Вы выбрали способ оплаты "на карту Сбербанка", то Вам нужно оплатить 100% стоимость заказа (полная предолата) на нашу карту Сбербанка - <b>4276 4000 5370 5670</b></p>
        <?php
      }
      break;
    case 5:
        ...
        <div class="order-total">
          <ul class="total-list">
            ...
            <?php if ($order['delivery_cost']): ?>
              ...
              <span><?php echo MG::numberFormat($order['delivery_cost']) . '  ' . $perCurrencyShort; ?></span></li>
            <?php endif; ?>
