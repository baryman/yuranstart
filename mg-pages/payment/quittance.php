...
<?php }else{
	...
	?>
	  <br/>
	  <a href="<?php echo SITE?>/order?getOrderPdf=<?php echo $data['id']?>" >Скачать счет в PDF</a>

  <?php }else{?>
      <br/>Скачать счет из личного кабинета, можно будет после подтверждения заказа, перейдя по ссылке в высланном вам письме.
 <?php }
} ?>
</div>