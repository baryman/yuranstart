...
<div class="products-wrapper brands">   
    ...
    $brand = $item['brand'];
    $pager = $item['pager'];
    $data['meta_title']= $brand['seo_title'];
    $data['meta_keywords'] = $brand['seo_keywords'];
    $data['meta_desc'] = $brand['seo_desc'];
    mgSEO($data);
    ?>
<?php if (!empty($brand)) { ?>
      <h1 class="new-products-title">Производитель: <?php echo $brand['brand'] ?></h1> 
      <div class="cat-desc">	        
    ...
    foreach ($items as $item) {
      ...
      if (!empty($imagesUrl[0])) {
        ...
        <div class="product-wrapper">
            ...
            </div>			<div class="product-description">          					<?php echo $item['thisUserFields']['42']['value']?>                 					</div>
            <span class="product-price"><?php echo priceFormat($item["price"]) ?> <?php echo $currency; ?></span>			<div class="countViews"><span class="eye-count">[count-views id=<?php echo $item['id']; ?>]</span> </div>
